...
class CallbackSender
{
    ...
    public function send(string $messageId, string $status, array $metadata = []): bool
    {
        ...
        $this->logger->info('[WhatsApp] 📤 Enviando callback de status', [
            'message_id' => $messageId,
            'status' => $status,
     ...
     */
    public function sendDeliveredWithDelay(string $messageId, int $minDelay = 1, int $maxDelay = 3): bool
    {
        $delay = rand($minDelay, $maxDelay);
        
        $this->logger->debug('[WhatsApp] ⏳ Aguardando {delay}s antes de DELIVERED', [
            'message_id' => $messageId,
            'delay' => $delay
        ]);
     ...
     * @param int $maxDelay Delay máximo em segundos
     */
    public function sendReadWithDelay(string $messageId, int $minDelay = 3, int $maxDelay = 8): bool
    {
        $delay = rand($minDelay, $maxDelay);
        
        $this->logger->debug('[WhatsApp] ⏳ Aguardando {delay}s antes de READ', [
            'message_id' => $messageId,
            'delay' => $delay
        ]);
    ...
    private function sendWithRetry(array $payload): bool
    {
        ...
        while ($attempt < $this->maxRetries) {
            try {
                ...
                if ($result) {
                    $this->logger->info('[WhatsApp] ✅ Callback enviado com sucesso', [
                ...
                $this->logger->warning('[WhatsApp] ⚠️ Falha ao enviar callback', [
                    'message_id' => $payload['message_id'],
                    'attempt' => $attempt + 1,
                    'error' => $e->getMessage()
                ]);
            ...
            if ($attempt < $this->maxRetries) {
                ...
                $this->logger->debug('[WhatsApp] 🔄 Retry em {delay}s', [
                    'attempt' => $attempt + 1,
                    'delay' => $delay
                ]);
                sleep($delay);
        ...
        $this->logger->error('[WhatsApp] ❌ Callback falhou após {maxRetries} tentativas', [
            'message_id' => $payload['message_id'],
            'status' => $payload['status'],
    ...
    private function doSend(array $payload): bool
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'X-Connector: whatsapp',
                'X-Message-ID: ' . $payload['message_id'],
                'User-Agent: Chat4All-WhatsApp-Connector/1.0'
            ],
        ...
        $this->logger->warning('[WhatsApp] HTTP {code} ao enviar callback', [
            'code' => $httpCode,
